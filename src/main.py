...
def parse_args(args=None):
    parser = argparse.ArgumentParser(description="RNNLogic", usage="train.py [<args>] [-h | --help]")
    ...
    parser.add_argument(
        ...
        action="store_true",
        default=True,
        help="disable the adversarial negative sampling",
    )
...
def _set_default(args, key, value):
...
def main():
    ...
    if args.init_checkpoint_config:
        args = load_config(args.init_checkpoint_config)[0]
        if not isinstance(args, argparse.Namespace):
            args = argparse.Namespace(**dict(args))

    _set_default(args, "use_adapter", False)
    _set_default(args, "use_logic_attention", False)
    _set_default(args, "attn_dim", 64)
    _set_default(args, "adapter_hidden_dim", 1024)
    _set_default(args, "adapter_dropout", 0.0)
    _set_default(args, "attn_dropout", 0.0)
    ...
    pre_trainer.evaluate("valid", expectation=True)
    pre_trainer.evaluate("test", expectation=True)
