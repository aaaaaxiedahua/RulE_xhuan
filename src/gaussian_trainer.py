"""
Gaussian-RulE: Trainer Implementation
训练器实现 - 两阶段训练流程

阶段1: WarmupTrainer - 几何预热（初始化高斯嵌入）
阶段2: JointTrainer - 联合训练（KGE + Rules）
"""
...
class WarmupTrainer:
    ...
    def train_step(self, batch):
        ...
        # KL 正则化（防止方差退化）
        logvars = self.model.entity_logvar_emb.weight
        loss_kl_reg = self.model.kl_regularization(logvars)

        # 总损失
        loss_total = loss_kge + loss_kl_reg
        ...
        while step < max_steps:
            for batch in self.train_dataloader:
                ...
                if step % 100 == 0 and step > 0:
                    ...
                    avg_loss_kl_reg = sum([l['loss_kl_reg'] for l in training_logs]) / len(training_logs)
                    ...
                    logging.info(f'[Warmup] Step {step}/{max_steps}: '
                               ...
                               f'loss_kl_reg={avg_loss_kl_reg:.6f}, '
...
class JointTrainer:
    ...
    def train_step(self, batch_triple, batch_rule):
        ...
        # 计算 Rule 分数
        negative_rule_score = self.model.compute_ruleE(
            (positive_rule, rule_mask, negative_idx, negative_rule),
            mode=mode_rule
        )
        positive_rule_score = self.model.compute_ruleE((positive_rule, rule_mask), mode='single')
        ...
        positive_rule_score = F.logsigmoid(positive_rule_score).squeeze(dim=1)
        ...
        loss_total = loss_kge + weight_rule * loss_rule + loss_kl_reg
