"""
Gaussian-RulE: Main Training Script
主训练脚本 - 两阶段训练流程

使用高斯分布嵌入替代盒子嵌入，解决梯度消失问题
"""
...
from gaussian_model import GaussianRulE
from gaussian_trainer import WarmupTrainer, JointTrainer
...
def parse_args():
    ...
    解析命令行参数
    """
    parser = argparse.ArgumentParser(description='Gaussian-RulE Training')
    ...
    parser.add_argument('--save_path', type=str, default='../checkpoints/gaussian_kinship', help='模型保存路径')
    ...
    parser.add_argument('--init_logvar', type=float, default=-1.0, help='初始log方差 (log(0.37)≈-1.0)')
    ...
    parser.add_argument('--lambda_kl', type=float, default=0.001, help='KL正则化权重')
    parser.add_argument('--target_logvar', type=float, default=0.0, help='目标log方差 (log(1.0)=0)')
...
def main():
    ...
    logging.info('='*50)
    logging.info('Gaussian-RulE Training')
    logging.info('='*50)
    logging.info(f'Using device: {args.device}')
    logging.info(f'Hidden dimension: {args.hidden_dim}')
    logging.info(f'Initial log-variance: {args.init_logvar}')

    # 加载数据
    graph, train_dataset, rule_dataset = load_data(args)

    # 初始化模型
    logging.info('Initializing Gaussian-RulE model...')
    model = GaussianRulE(graph, args)
    ...
    logging.info('')
    logging.info('='*50)
    logging.info('Stage 1: Warmup Training (Gaussian Initialization)')
    logging.info('='*50)
    logging.info('目标：初始化高斯嵌入，防止方差退化')
    ...
    logging.info('='*50)
    logging.info('目标：学习规则嵌入，动态置信度 w_i(h)')
