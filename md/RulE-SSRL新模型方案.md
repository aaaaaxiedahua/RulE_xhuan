...
| **训练方式** | RulE: 预训练→Grounding<br/>SSRL: BFS标签→SL→RL | **端到端训练**：<br/>• 联合训练阶段学习所有参数<br/>• 可选RL微调进一步优化 | ✓ 简化训练流程<br/>✓ 避免阶段间的信息损失 |
| **推理方式** | RulE: Grounding传播<br/>SSRL: 多次采样 | **策略网络多次采样**：<br/>• K次路径探索<br/>• 规则嵌入提供探索指引<br/>• 聚合K条路径结果 | ✓ 去掉Grounding模块<br/>✓ 统一的推理接口 |

**关键优势**：
- **不是叠加**：不是"先RulE预训练，再SSRL训练"，而是联合优化
- **规则嵌入有用武之地**：直接在策略网络中作为"建议者"参与决策
- **架构简洁**：两阶段训练（联合训练+可选RL微调）取代五阶段
...
│  │  │         + rule_context (从rule_emb聚合)                    │ │   │
│  │  │                                                             │ │   │
│  │  │  网络: LSTM + Rule Attention + Action Selector             │ │   │
│  │  │                                                             │ │   │
│  │  │  输出: action_prob = π(a|s, rules)                          │ │   │
│  │  │                                                             │ │   │
│  │  │  训练: L_total = L_KGE + L_Rule + L_Policy                 │ │   │
│  │  └─────────────────────────────────────────────────────────────┘ │   │
│  └──────────────────────────────────────────────────────────────────┘   │
│                                  │                                      │
│                                  ▼                                      │
│  ┌──────────────────────────────────────────────────────────────────┐   │
│  │          Phase 2: RL微调（可选，用于进一步优化）                 │   │
│  │                                                                   │   │
│  │  目的：在Phase 1基础上，用自由探索进一步提升                      │   │
│  │                                                                   │   │
│  │  操作：                                                           │   │
│  │  • 冻结嵌入层（entity_emb, relation_emb, rule_emb）              │   │
│  │  • 用Policy Gradient微调策略网络                                 │   │
│  │  • 使用混合奖励: R = R_term + λ·R_rule                           │   │
│  │                                                                   │   │
│  │  作用：                                                           │   │
│  │  • 突破规则限制，发现新路径                                       │   │
│  │  • 用真实探索反馈优化策略                                         │   │
...
1. **两阶段训练**: 联合训练（嵌入+策略） → 可选RL微调
2. **规则注意力机制**: 神经网络学习何时采纳哪条规则的建议
3. **端到端优化**: 去掉Grounding模块，统一用策略网络推理
